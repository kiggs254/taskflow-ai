...
**A tone is not content.** `Shorter`/`Firmer` and the free-text box are directives about *wording*; they are never authority to originate a reply. The guard was once "no draft AND no instruction", and a tone chip always supplies an instruction — so an empty editor reached the model, which (with a schema demanding a complete reply body) invented an entire message to a client from the one-line summary, landed it in a send-ready editor, and enabled Send. Every layer now requires actual text: the card, the route, and `rewriteReply` itself, which returns null before any AI call. Relatedly, the editor sends `currentDraft` **only when it still holds the AI's draft** — once the user replaces it, their text is the `notes` and the rejected draft is the stored one. Sending their own words back labelled *"the user was not happy with this"* directly above *"follow these exactly"* told the model to change the very content it was meant to preserve, which is how a client-facing "no" gets softened.
...
- **Integration-sourced work is idempotent by construction.** GitHub commits become one task per repo **per branch** per local day with the deterministic id `gh-{uid}-{repoId}-{branchSlug}-{day}`; `syncTask` upserts on that id, and each task's subtasks are rebuilt from `processed_commits` (grouped by the `branch` column, legacy nulls falling back to the default branch) rather than from the API response. The non-default branch is shown in the label (`repo (branch)`). Commits re-point to their branch task **by SHA**, and `scanCommits` prunes any `gh-` task left un-backed (e.g. the old per-repo-day task before branches were split). Don't switch to `crypto.randomUUID()` here — the determinism is what makes a re-scan safe.
- **An installation's account is not a commit author, and there can be more than one account.** `github_integrations` holds one row per connected GitHub account, keyed `UNIQUE(user_id, installation_id)` — it used to be `UNIQUE(user_id)`, so installing the app on an org silently *replaced* the personal account's row and stranded its repos on an installation that could no longer be authenticated. Each row carries `account_login`/`account_type` (who the app is installed on, straight from `GET /app/installations/{id}`) and `author_login` (whose commits count as the user's work). These were once the same column, set to `repos[0].owner.login`, and used as the commits API's `&author=` filter — so transferring repos to an organisation made it the *org* name, an org cannot author a commit, and every scan quietly returned zero. `author_login` is only ever auto-set when `account_type === 'User'`; an org installation contributes nothing and borrows the set built from the user's personal accounts or typed into Settings. `authorLoginsFor` returns that set; `&author=` takes one value so it's used only when the set has exactly one entry, and `authoredBy` filters locally otherwise. An **empty** set means no filter at all — over-report visibly rather than under-report silently.
- **Clients are per installation, never per user.** `getClientForIntegration(row)`, not `getClientForUser(id)`: with several accounts connected, one per-user client authenticated every repo against whichever row sorted first and 404'd on all the others. `github_repos.installation_id` records which account grants each repo, and `scanCommits`/`getCommitMetrics` carry the right client alongside each repo.
- **A repo that goes out of reach is tombstoned, never deleted.** `github_repos.access_lost_at` is set when an installation stops returning it (`markLostRepos`) or a scan 404s on it — GitHub answers 404, not 403, for a repo an installation cannot see. Deleting the row instead would lose `selected` and, because `repo_id` is the identity behind `processed_commits` and every `gh-{uid}-{repoId}-...` id, re-ingest the entire history if the repo ever came back. **A transfer keeps the numeric id**, so `upsertRepos` re-homes the row to the new owner's installation (`installation_id = EXCLUDED.installation_id`, `access_lost_at = NULL`) and the commit ledger, the task ids and the tracked-repo selection all survive — connecting the org account is the whole fix. `findCoveredRepo` skips tombstoned repos: a repo the scanner can't reach isn't covering anything, and treating it as covered drops the agent session log too, leaving the work recorded nowhere.
- **The GitHub scanner covers every branch, not just the default.** The commits API's `sha` param selects a single ref (defaulting to the default branch), so `fetchRepoCommits` enumerates branches (`listBranches`, capped at `MAX_BRANCHES`) and unions each branch's `since`+`author`-filtered commits, deduped by SHA (then again by the `processed_commits` unique index). Feature-branch work was invisible until it merged before this. It trades away the per-repo ETag 304 short-circuit; at the 30-min default scan frequency that's a few small requests per repo, far inside the 15k/hr App budget. A rate-limit aborts the repo; a branch deleted mid-scan is skipped.
...
- **The session heading names the client, and the client is never guessed.** `sitesInPrompts` pulls hostnames out of the recorded prompts with a regex *before* the summariser runs, and `SUMMARY_SCHEMA` orders `deliverable` and `site` **before** `project` so the model decides what was built and whose system it was on rather than rationalising a heading after the fact — asking for the heading first produced "WhatsApp button on product pages" for a session that shipped a plugin to a named client. The site is extracted deterministically because it is usually one URL buried in a 500-char prompt (the recorder truncates each) beside a paste of PHP; a filename or version number is not a site: extensions are denylisted AND the TLD is allowlisted (any two letters, i.e. every ccTLD, plus the named gTLDs), because a denylist alone is unbounded — a scratch file called `blh3jaemh.output` was published as a client's website, and the next one would have been `.bak` or `.orig`. The extension check runs first, or two-letter extensions like `.js` and `.go` would pass as ccTLDs. `sitesInPrompts` takes `{strict}` because its two callers want opposite strictness: **extraction** from prompts must not promote a scratch file to a client name, while **scrubbing** a hostname out of a heading must catch anything host-shaped — a false positive there removes an odd-looking token, a false negative publishes a client the model invented. `composeProjectLabel` then **verifies** the model's answer: a `site` outside the grounded list is dropped, and any hostname the model wrote into the heading text itself is stripped out. A wrong client name in a report a manager checks against real records is worse than no client name. It also guarantees the heading holds no `" — "`, since `splitProjectTitle` cuts the stored title on the first one and a heading containing it would swallow the summary.
- **Shell commands are the record of what was built; prompts are only the record of what was asked.** `taskflow-record.mjs` hooks `Edit|Write|Bash` because most work never touches Edit/Write — a plugin written with a heredoc, a WP-CLI call, a deploy. Two real sessions each shipped a plugin and recorded zero files, so their summaries could only ever describe the request. Commands are redacted by `redact()` **before they are written to the local log**, so a credential is never persisted either: assignments and flags first (so a narrower rule can't match and keep the value), then headers, URL credentials, known key prefixes, PEM blocks, and any bare 32-char high-entropy blob. It is deliberately over-eager — a redacted flag costs a vaguer summary, a missed key is a leaked credential — and it is not complete, which is why the work-folder allowlist still gates everything. `backend/test/redactCommand.test.js` loads the function out of the shipped hook's own text so an installed copy that drifts is caught. Commands are persisted to `agent_sessions.commands` rather than used and discarded, or `resummarise` would replay a session with less evidence than the original had.
...
- **Work with no digital trace still needs a way in.** Everything else in the day's record arrives on its own — commits from the GitHub scanner, Claude Code sessions from the SessionEnd hook — so when the task list was removed there was no longer *any* way to record a call, an SSH fix or work done in a client's dashboard, and none of it could reach the 16:30 report. `LogWorkModal` (home screen, "Log work done elsewhere") creates a task already `done` with `completedAt: now`, which is what puts it in the window; without that timestamp it is done but belongs to no day and reaches no report. It stores the title as `${project} — ${outcome}` deliberately: `splitProjectTitle` cuts on that separator and `narrateItem`'s no-subtasks path returns the outcome unchanged, so what the user types is exactly what the channel reads — no AI call, nothing invented. The id is a plain uuid, unlike the deterministic `gh-`/`agent-` ids, because nothing re-derives a hand entry and a collidable id would let a later entry silently replace it.
- **A correction to the report lives where nothing regenerates it.** Both halves of a report line are derived — the heading from `splitProjectTitle(tasks.title)`, the paragraph from `narrateItem` — and both are *rewritten*: a GitHub or agent task is rebuilt by `syncTask` on every scan and every re-flush, and End Day Reset passes `refresh:true` to re-narrate deliberately. So an edit to `tasks.title` would survive until the next scan and no longer. `tasks.report_title` / `tasks.report_narrative` are the override: `syncTask`'s `ON CONFLICT` list does not mention them, so a rebuild leaves them alone, and `attachNarratives` prefers them over anything derived **including under refresh** — otherwise fixing a wrong heading would last until the next wrap-up. An override also skips the AI call entirely; there is nothing left to ask. Whitespace is not an override (it would blank the line rather than restore the generated one), and `''` clears it, handing the item back to the generators. `setReportOverride` is scoped by `user_id`: task ids are guessable (`agent-{uid}-{session}-{day}`), so the id alone must never authorise a write. The same override feeds the panel, the Settings preview and the 16:30 send, which is the whole point — the edit changes what goes out, not what is on screen.
